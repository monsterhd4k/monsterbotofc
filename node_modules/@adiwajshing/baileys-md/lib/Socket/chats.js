...
const makeChatsSocket = (config) => {
    ...
    const updateProfilePicture = async (jid, content) => {
        const { img } = await (0, Utils_1.generateProfilePicture)('url' in content ? content.url.toString() : content);
        await query({
            tag: 'iq',
            attrs: {
    ...
    const appPatch = async (patchCreate) => {
        ...
        await mutationMutex.mutex(async () => {
            ...
            const { patch, state } = await (0, Utils_1.encodeSyncdPatch)(patchCreate, authState);
